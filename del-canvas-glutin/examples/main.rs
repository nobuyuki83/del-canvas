...
impl del_canvas_glutin::Renderer for Renderer {
    fn new<D: GlDisplay>(gl_display: &D) -> Self {
        unsafe {
            ...
            if let Some(renderer) = del_canvas_glutin::get_gl_string(&gl, gl::RENDERER) {
                println!("Running on {}", renderer.to_string_lossy());
            }
            if let Some(version) = del_canvas_glutin::get_gl_string(&gl, gl::VERSION) {
...
fn main() -> Result<(), Box<dyn Error>> {
    del_canvas_glutin::main::<Renderer>(EventLoop::new().unwrap())
}