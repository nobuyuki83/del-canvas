...
fn main() -> anyhow::Result<()> {
    ...
    let vtx2xyzrgb = del_msh_core::io_ply::read_xyzrgb::<_, XyzRgb>(file_path)?;
...

    {
        ...
        for i_idx in 0..vtx2xyzrgb.len() {
            let i_vtx = idx2vtx[i_idx];
            let r0 = vtx2splat[i_vtx].pos_pix;
            let ix = r0[0] as usize;
            let iy = r0[1] as usize;
            let ipix = iy * img_shape.0 + ix;
            img_data[ipix][0] = (vtx2xyzrgb[i_vtx].rgb[0] as f32) / 255.0;
            img_data[ipix][1] = (vtx2xyzrgb[i_vtx].rgb[1] as f32) / 255.0;
    ...
    }
    {
        ...
        use del_msh_core::vtx2xyz::HasXyz;
        for idx in 0..vtx2xyzrgb.len() {
            let i_vtx = idx2vtx[idx];
            let p0 = vtx2xyzrgb[i_vtx].xyz();
            let p0: [f32; 3] = p0.map(|v| v.as_());
            ...
            for i_pix in pixs {
                img_data[i_pix][0] = (vtx2xyzrgb[i_vtx].rgb[0] as f32) / 255.0;
                img_data[i_pix][1] = (vtx2xyzrgb[i_vtx].rgb[1] as f32) / 255.0;
                img_data[i_pix][2] = (vtx2xyzrgb[i_vtx].rgb[2] as f32) / 255.0;
            }
        }
        use ::slice_of_array::SliceFlatExt; // for flat
        del_canvas_cpu::write_png_from_float_image_rgb(
    ...
    }
    {
        ...
        let mut num_ind = 0;
        for i_vtx in 0..vtx2splat.len() {
            let p0 = vtx2splat[i_vtx].pos_pix;
            let rad = vtx2splat[i_vtx].rad_pix;
