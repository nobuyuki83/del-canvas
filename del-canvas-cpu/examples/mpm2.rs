...
pub fn myclamp<T>(v: T, vmin: T, vmax: T) -> T
where
...
fn add_object(particles: &mut Vec<Particle>, center: Vector, c: u8, rng: &mut rand::rngs::StdRng) {
    use rand::Rng;
    for _i in 0..1000 {
    ...
    for p in particles {
        ...
        for i in 0..3 as usize {
            for j in 0..3 as usize {
                let dpos = Vector::new((i as Real - fx.x) * dx, (j as Real - fx.y) * dx);
                ...
                grid[ih * (n + 1) + iw] += w[i].x * w[j].y * (mass_x_velocity + t);
            }
        }
    }
}
...
fn mpm2_grid2particle(
    ...
    for p in particles {
        ...
        for i in 0..3 {
            for j in 0..3 {
                let dpos = Vector::new(i as Real - fx.x, j as Real - fx.y);
                let ig = (base_coord.x + i as i32) as usize;
                let jg = (base_coord.y + j as i32) as usize;
                assert!(ig < mgrid);
                assert!(jg < mgrid);
                let grid_v = Vector::new(grid[jg * mgrid + ig].x, grid[jg * mgrid + ig].y);
        ...
        if is_plastic {
            // updating deformation gradient tensor by clamping the eignvalues
            ...
            for i in 0..2 {
                // Snow Plasticity
                sig0[(i, i)] = myclamp(sig0[(i, i)], 1.0 - 2.5e-2, 1.0 + 7.5e-3);
            }
            F0 = svd_u0 * sig0 * svd_v0.transpose();
        }
        let Jp_new0: Real = myclamp(p.Jp * oldJ0 / F0.determinant(), 0.6, 20.0);
        p.Jp = Jp_new0;
        p.F = F0;
    }
}
...
fn main() {
    ...
    const N: usize = 80;
    const M: usize = N + 1;
    let mut grid: Vec<nalgebra::Vector3<Real>> =
    ...
    let transform_world2pix: [Real; 9] = nalgebra::Matrix3::<Real>::new(
        canvas.width as Real,
        0.,
        0.,
        0.,
        -(canvas.height as Real),
        canvas.height as Real,
        0.,
        0.,
        1.,
    ...
    for i_step in 0..2000 {
        ...
        mpm2_grid2particle(&mut particles, &grid, N, DT, true);
        if i_step % ((FRAME_DT / DT) as i32) == 0 {
            canvas.clear(0);
            for p in particles.iter() {
                del_canvas_cpu::rasterize_circle::fill::<Real, u8>(
                    &mut canvas.data,
                    canvas.width,
                    &[p.x.x, p.x.y],
                    &transform_world2pix,
