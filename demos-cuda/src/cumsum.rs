...
fn block_sums(
    ...
    d_in: &CudaSlice<u32>,
    num_elem: u32) -> anyhow::Result<(u32, u32, CudaSlice<u32>)>
{
    ...
    //gpu_sum_scan_blelloch<<<grid_sz, block_sz, sizeof(unsigned int) * max_elems_per_block >>>(d_out, d_in, d_block_sums, numElems);
    {
        ...
        let gpu_prescan = dev.get_func("cumsum", "gpu_prescan").unwrap();
    ...
    if grid_size <= max_elems_per_block {
        ...
        let (grid_sz1, block_sz1, d_block_sums1) = block_sums(dev, &mut d_block_sums, &d_in_block_sums, grid_size)?;
        add_block_sums(dev, &mut d_block_sums, &d_block_sums1, num_elem, grid_sz1, block_sz1)?;
    }
    ...
    block_sz:u32) -> anyhow::Result<()>
{
    ...
    let param = (
        ...
        num_elem);
    use cudarc::driver::LaunchAsync;
    let gpu_add_block_sums = dev.get_func("cumsum", "gpu_add_block_sums").unwrap();
...
pub fn sum_scan_blelloch(
    dev: &std::sync::Arc<CudaDevice>,
    vout_dev: &mut CudaSlice<u32>,
    vin_dev: &CudaSlice<u32>) -> anyhow::Result<()>
{
    let n = vin_dev.len();
...
fn test() -> anyhow::Result<()> {
    ...
    for (n,v) in nvs {
        ...
        for i in 0..n {
            assert_eq!(vout[i+1] as i64 -vout[i] as i64, vin[i] as i64, "{}-->{} {}",i, vout[i], vout[i+1]);
        }
