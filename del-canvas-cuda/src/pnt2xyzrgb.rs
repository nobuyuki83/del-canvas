...
pub fn tile2idx_idx2pnt(
    ...
    tile_shape: (u32, u32),
    pnt2splat_dev: &cudarc::driver::CudaSlice<Splat>,
) -> anyhow::Result<(
    ...
    let (tile2idx_dev, pnt2ind_dev) = {
        ...
        let count_splat_in_tile =
            crate::get_or_load_func(&dev, "count_splat_in_tile", del_canvas_cuda_kernel::XYZRGB)?;
    ...
    let idx2pnt_dev = {
        ...
        use cudarc::driver::LaunchAsync;
        let count_splat_in_tile =
            crate::get_or_load_func(&dev, "fill_index_info", del_canvas_cuda_kernel::XYZRGB)?;
        unsafe { count_splat_in_tile.launch(cfg, param) }?;
...
pub fn pnt2xyzrgb_to_pnt2splat(
    dev: &std::sync::Arc<cudarc::driver::CudaDevice>,
    pnt2xyzrgb_dev: &cudarc::driver::CudaSlice<XyzRgb>,
    pnt2splat_dev: &mut cudarc::driver::CudaSlice<Splat>,
    transform_world2ndc_dev: &cudarc::driver::CudaSlice<f32>,
    img_shape: (u32, u32),
    radius: f32,
) -> anyhow::Result<()> {
    let cfg = cudarc::driver::LaunchConfig::for_num_elems(pnt2xyzrgb_dev.len() as u32);
    let param = (
        pnt2xyzrgb_dev.len(),
        pnt2splat_dev,
        pnt2xyzrgb_dev,
    ...
    use cudarc::driver::LaunchAsync;
    let xyzrgb_to_splat =
        crate::get_or_load_func(&dev, "xyzrgb_to_splat", del_canvas_cuda_kernel::XYZRGB)?;
