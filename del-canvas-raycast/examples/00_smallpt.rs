...
fn radiance(r: &Ray, depth: i64, spheres: &[Sphere], rng: &mut rand::rngs::ThreadRng) -> Vector {
    ...
    let next_dir = match obj.refl {
        ReflT::DIFF => del_canvas_raycast::sampling::hemisphere_cos_weighted(
            &hit_nrm,
...
fn main() {
    ...
    let samps = 5;
    let cam = del_canvas_raycast::cam3::Camera3::new(
        1024,
        768,
        ...
        for x in 0..cam.w {
            ...
            for sy in 0..2 {
                ...
                for sx in 0..2 {
                    // 2x2 subpixel cols
                    for _s in 0..samps {
                        let dx = del_canvas_raycast::sampling::tent(rng.gen::<Real>());
                        let dy = del_canvas_raycast::sampling::tent(rng.gen::<Real>());
                        let sx = sx as Real;
